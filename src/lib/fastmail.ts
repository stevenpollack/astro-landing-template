...
async function resolveSendContext(
  ...
  const { methodResponses } = await jmapRequest(session, token, [
    ...
    ["Mailbox/query", { accountId: session.accountId, filter: { role: "drafts" } }, "m"],
  ]);

  const identities =
    (methodResponses.find((r) => r[2] === "i")?.[1].list as Identity[] | undefined) ?? [];
  const match =
    identities.find((id) => id.email?.toLowerCase() === from.toLowerCase()) ?? identities[0];
  if (!match?.id) {
...
export async function sendEmail(token: string, input: SendEmailInput): Promise<void> {
  const session = await getSession(token);
  const { identityId, draftsId } = await resolveSendContext(session, token, input.from);
  ...
  for (const [name, args] of methodResponses) {
    ...
    const notCreated = (args as { notCreated?: Record<string, unknown> }).notCreated;
