...
export const POST: APIRoute = async ({ request, clientAddress }) => {
	...
	if (env.TURNSTILE_SECRET_KEY) {
		...
		const passed =
			token && (await verifyTurnstile(env.TURNSTILE_SECRET_KEY, token, ip));
		if (!passed) {
	...
	if (!env.FASTMAIL_API_TOKEN || !env.MAIL_FROM || !env.MAIL_TO) {
		console.error(
			"Contact form is missing FASTMAIL_API_TOKEN / MAIL_FROM / MAIL_TO",
		);
		return fail(
			500,
	...
	try {
		...
		return fail(
			502,
			"Sorry — we couldn't send your message. Please try again shortly.",
		);
	}
