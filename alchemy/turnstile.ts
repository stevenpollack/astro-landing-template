...
export const TurnstileWidget = Resource(
  ...
  async function (
    ...
    if (!token || !accountId) {
      throw new Error("TurnstileWidget requires CLOUDFLARE_API_TOKEN and CLOUDFLARE_ACCOUNT_ID.");
    }
    ...
    if (this.phase === "delete") {
      ...
      if (sitekey) {
        ...
        if (!res.ok && res.status !== 404) {
          throw new Error(`Failed to delete Turnstile widget ${sitekey} (${res.status}).`);
        }
      }
      return this.destroy();
    }
    ...
    const domains = [...new Set([...props.domains, host].filter((d): d is string => !!d))];
    ...
    // Create.
    const result = await cfFetch<{ sitekey: string; secret: string }>(base, token, {
      method: "POST",
      body: payload,
    });
    return {
